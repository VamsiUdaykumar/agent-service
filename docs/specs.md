...
- **Error model:** one envelope everywhere — `{type, code, message, param, request_id}`. `type` is a closed set (`invalid_request`, `not_found`, `conflict`, `idempotency_error`, `internal_error`); `code` is the growable specific case. FastAPI's default 422 handler is overridden to match. `request_id` on every response and on the trace — the support-ticket-to-trace link. 5xx bodies never leak internals.
...
| **Runner** (`app/runner`) | The fake agent: steps, latency, tokens, cost, failures, retries, sub-agents | Three profiles as one config dict (researcher 5–8 steps/10% fail; simple 2–3/2%; flaky 4–6/35% + 5% non-retryable); all randomness from the recipe-seeded RNG; `SIM_SPEED` scales sleeps only, recorded durations stay simulated |
