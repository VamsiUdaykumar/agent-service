...
async def _collect(agent_id: str, seed: int, input: dict | None = None, **kwargs):
    events = []
    async for event in execute_run(
        ...
        seed=seed,
        input=input if input is not None else {"prompt": "task"},
        sim_speed=FAST,
        **kwargs,
    ):
        events.append(event)
...
async def test_event_stream_starts_with_run_started() -> None:
    events = await _collect("agent-simple", 0)
...
async def test_failed_run_ends_with_run_failed_and_includes_failed_attempt_tokens() -> None:
    # Seed 0 with input {"prompt": "task"} (the _collect default) exhausts
    # flaky's retries.
    events = await _collect("agent-flaky", 0)
...
async def test_step_started_carries_incrementing_attempt_on_retry() -> None:
    ...
    events = await _collect("agent-researcher", 1)
...
async def test_sub_agent_children_carry_parent_step_id() -> None:
    ...
    for seed in range(15):
        events = await _collect("agent-researcher", seed)
        sub_agent_ids = {
            e.step_id
            for e in events
...
async def test_cancel_before_first_step_boundary_stops_after_run_started() -> None:
    signal = CancelSignal()
    signal.set()
    events = await _collect("agent-researcher", 0, cancel_signal=signal)
